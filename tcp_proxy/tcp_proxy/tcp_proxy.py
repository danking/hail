...
from hailtop.tls import get_in_cluster_client_ssl_context, get_in_cluster_server_ssl_context
...
CLIENT_TLS_CONTEXT = get_in_cluster_client_ssl_context()
...
async def handle(source_reader: asyncio.StreamReader, source_writer: asyncio.StreamWriter):
    ...
    if '.' in service_name or '.' in namespace_name:
        log_info(f'malformed request: {namespace_name} {service_name}')
        source_writer.write(b'\x00')
        source_writer.close()
        return

    hostname = f'{service_name}.{namespace_name}'
    request_metadata['target_addr'] = (hostname, port)

    try:
        if namespace_name == 'default':
            client_tls_context = CLIENT_TLS_CONTEXT
        else:
            # do not verify namespaced certs
        ...
        log_info(f'could not connect to {hostname}:{port}', exc_info=True)
        source_writer.write(b'\x00')
        source_writer.close()
        return
    ...
    target_writer.write(request_id.bytes)
