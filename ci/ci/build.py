...
class CreateDatabaseStep(Step):
    ...
    def wrapped_job(self):
        ...
        if self.create_password_job:
            assert self.create_database_job is not None
            return [self.create_password_job, self.create_database_job]
        return []
...
'''
        ...
        password_files_input = [
            (f'{BUCKET}/build/{batch.attributes["token"]}/{self.admin_password_file}', self.admin_password_file),
            (f'{BUCKET}/build/{batch.attributes["token"]}/{self.user_password_file}', self.user_password_file)]
        input_files.extend(password_files_input)

        self.create_password_job = batch.create_job(
            CI_UTILS_IMAGE,
            command=['bash', '-c', create_password_script],
            attributes={'name': self.name},
            secrets=[{
