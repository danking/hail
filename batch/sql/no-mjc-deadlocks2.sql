...
BEGIN
  ...
  ELSEIF cur_job_state = 'Ready' OR cur_job_state = 'Creating' OR cur_job_state = 'Running' THEN
    ...
    SET jobs.state = new_state,
        ...
        batches.n_cancelled = n_cancelled + (new_state = 'Cancelled')
        batches.n_failed = n_failed + (new_state = 'Error' OR new_state = 'Failed')
        batches.n_succeeded = n_succeeded + (new_state = 'Success')
