...
from hailtop.tls import get_context_specific_ssl_client_session, check_ssl_config
...
class Worker:
    ...
    async def activate(self):
        async with get_context_specific_ssl_client_session(
            ...
            os.mkdir('/ssl-config')
            ssl_config = resp_json['ssl_config']
            for fname, data in ssl_config.items():
                open('/ssl-config/' + fname).write(data)
            assert 'ssl-config.json' in ssl_config
