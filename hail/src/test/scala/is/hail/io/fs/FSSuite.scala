...
trait FSSuite extends TestNGSuite {
  ...
  @Test def testFileListEntryOnMissingFile(): Unit = {
    TestUtils.interceptException[FileNotFoundException](r("/does_not_exist"))(
  ...
  @Test def testFileListEntryOnDirWithSlashIsFailure(): Unit = {
    TestUtils.interceptException[FileNotFoundException](r("/dir/"))(
      r("/dir/")
    )
  }
  ...
  @Test def fileListEntryForFileAndDirectoryIsError(): Unit = {
    val d = t()
    fs.mkDir(d)
    fs.touch(s"$d/x")
    fs.touch(s"$d/x/file")
  ...
  @Test def fileListEntryForFileAndDirectoryIsErrorEvenIfNotFirstEntryInList(): Unit = {
    val d = t()
    fs.mkDir(d)
    fs.touch(s"$d/x")
  ...
  @Test def fileListEntrySeesDirectoryEvenIfNotFirstEntryInList(): Unit = {
    ...
    val fileListEntry = fs.fileListEntry(s"$d/x")
    assert(fileListEntry.isDirectory)
    assert(!fileListEntry.isFile)
  }
