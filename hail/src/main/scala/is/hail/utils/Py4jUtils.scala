...
trait Py4jUtils {
  ...
  def ls(fs: FS, path: String): String = {
    val fileListEntries = fs.listDirectory(path)
    JsonMethods.compact(JArray(fileListEntries.map(fileListEntry => fileListEntryToJson(fileListEntry)).toList))
  }
  ...
  def fileListEntry(fs: FS, path: String): String = {
    val fileListEntry = fs.fileListEntry(path)
    JsonMethods.compact(fileListEntryToJson(fileListEntry))
  }
