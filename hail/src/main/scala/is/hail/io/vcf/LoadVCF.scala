      ...
      new Iterator[Long] {
        ...
        def hasNext: Boolean = {
          while (!present && it.hasNext) {
            ...
            val line = lwc.value
            try {
              val vcfLine = new VCFLine(line, context.fileNum, lwc.source.position.get, arrayElementsRequired, abs, abi, abf, abd)
              rvb.start(rowPType)
              rvb.startStruct()
  ...
  def apply(ctx: ExecuteContext, params: MatrixVCFReaderParameters): MatrixVCFReader = {
    ...
    if (fileListEntries.length > 1) {
      if (params.headerFile.isEmpty) {
        ...
        backend.parallelizeAndComputeWithIndex(ctx.backendContext, fs, files.tail.map(_.getBytes).zipWithIndex, "load_vcf_parse_header", None) { (bytes, htc, _, fs) =>
