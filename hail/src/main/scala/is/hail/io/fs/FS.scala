...
object FS {
  ...
  def fileListEntryFromIterator(
    ...
    var file: FileListEntry = null
    var dir: FileListEntry = null
    while (continue) {
      val fileListEntry = it.next()

      if (fileListEntry.isFile && fileListEntry.getActualUrl == urlStr) {
        // In Google, there could be a blob with the name "foo/". We return it iff the user
        // requested "foo/" with the trailing slash.
        file = fileListEntry
      }
      if (fileListEntry.isDirectory && dropTrailingSlash(fileListEntry.getActualUrl) == prefix) {
      ...
      continue = it.hasNext && (fileListEntry.getActualUrl <= prefixWithSlash)
    }

    if (file != null) {
      if (dir != null) {
        throw new FileAndDirectoryException(s"${url.toString} appears as both file ${file.getActualUrl} and directory ${dir.getActualUrl}")
      } else {
        file
      }
    } else {
...
trait FS extends Serializable {
  ...
  def copyMerge(
    ...
    val headerFiles = glob(sourceFolder + "/header")

    if (header && headerFiles.isEmpty)
      fatal(s"Missing header file")
    else if (!header && headerFiles.nonEmpty)
      fatal(s"Found unexpected header file")

    val partFiles: Array[_ <: FileStatus] = partFilesOpt match {
      case None => glob(sourceFolder + "/part-*")
      case Some(files) => files.map(f => fileStatus(sourceFolder + "/" + f)).toArray
    }

    val sortedPartFileListEntries = partFiles.sortBy { fileStatus =>
    ...
    val filesToMerge: Array[FileStatus] = headerFiles ++ sortedPartFileListEntries
  ...
  def copyMergeList(srcFileStatuses: Array[_ <: FileStatus], destFilename: String, deleteSource: Boolean = true) {
    ...
    require(srcFileStatuses.forall {
      srcFileStatus => srcFileStatus.getPath != destFilename
    })
    ...
    using(createNoCompression(destFilename)) { os =>
      ...
      while (i < srcFileStatuses.length) {
        val srcFileStatus = srcFileStatuses(i)
        val lenAdjust: Long = if (isBGzip && i < srcFileStatuses.length - 1)
          -28
        else
          0
        using(openNoCompression(srcFileStatus.getPath)) { is =>
          hadoop.io.IOUtils.copyBytes(is, os,
            srcFileStatus.getLen + lenAdjust,
            false)
        }
        i += 1
      }
    ...
    if (deleteSource) {
      srcFileStatuses.foreach { srcFileStatus =>
        delete(srcFileStatus.getPath, recursive = true)
      }
    }
  }
