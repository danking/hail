...
def _file_status_dict_to_file_status(file_status: Dict[str, Any]) -> FileStatus:
...
class HadoopFS(FS):
    ...
    def stat(self, path: str) -> FileListEntry:
        filestatus_dict = json.loads(self._utils_package_object.fileListEntry(self._jfs, path))
        return _file_list_entry_scala_to_python(filestatus_dict)
