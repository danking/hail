...
async def open_connection(
    ...
    if location == 'k8s':
        kwargs['session_ids'] = (session_id_decode_from_str(tokens.namespace_token_or_error(ns)),
                                 b'\x00' * 32)
        return await _retry_open_connection(f'{service}.{ns}',
                                            port,
                                            None,
                                            **kwargs)
    ...
    return await _retry_open_connection(proxy_hostname, 5000,
                                        proxy_to={'service': service, 'ns': ns, 'port': port},
                                        **kwargs)


async def _retry_open_connection(hostname: str,
                                 port: int,
    ...
    while True:
        ...
        try:
            ...
            log.info(f'end of file encountered before reading anything, retrying connection')
            continue
        if is_success != b'\x01':
