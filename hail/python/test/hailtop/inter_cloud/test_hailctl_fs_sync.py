...
from .copier_test_utilities import event_loop, test_spec, cloud_scheme, router_filesystem, fresh_dir
...
async def test_cli_file_and_dir(router_filesystem, cloud_scheme):
    ...
    await check_exec_output(
        'hailctl', 'fs', 'sync', '--make-plan',  plandir,
        '--copy-to', f'{test_dir}file1', f'{test_dir}file2',
        '--copy-into', f'{test_dir}file1', f'{test_dir}dir1',
    )

    await check_exec_output(
        'hailctl', 'fs', 'sync', '--use-plan',  plandir,
    )
    ...
    await check_exec_output(
        'hailctl', 'fs', 'sync', '--make-plan',  plandir2,
        '--copy-to', f'{test_dir}file1', f'{test_dir}file2',
        '--copy-into', f'{test_dir}file1', f'{test_dir}dir1',
    )
...
async def test_cli_subdir(router_filesystem, cloud_scheme):
    ...
    await check_exec_output(
        'hailctl', 'fs', 'sync', '--make-plan',  plandir,
        '--copy-to', f'{test_dir}dir', f'{test_dir}dir2'
    )
...
async def test_cli_already_synced(router_filesystem, cloud_scheme):
    ...
    await check_exec_output(
        'hailctl', 'fs', 'sync', '--make-plan',  plandir,
        '--copy-to', f'{test_dir}dir/foo', f'{test_dir}bar'
    )
